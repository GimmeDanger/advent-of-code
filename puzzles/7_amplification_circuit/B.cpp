...
static int pick_value(const vector<int>& memory, std::array<int, max_arg_num> &arg_modes, size_t arg_starting_pos) {
  ...
  if (EQUAL(mode, parameter_mode::position)) {
    assert(pos < memory.size());
    assert(memory[pos] < memory.size());
...
class Intcode {
 ...
 public:
  ...
  int eval(vector<int>::const_iterator first, vector<int>::const_iterator last) {
    ...
    while (ptr < memory.size() && !is_halt()) {
      ...
      if (EQUAL(instr, command::halt)) {
        ...
        assert(first != last);
        set_value<0>(memory, arg_modes, arg_starting_pos, *first);
...
int main() {
  ...
  do {
    input[0][1] = 0;
    vector<Intcode> cpus(input.size(), Intcode(initial_state));
    while (!cpus.back().is_halt()) {
      for (size_t i = 0; i < cpus.size(); i++) {
        long long output = cpus[i].eval(input[i].begin() + (int)cpus[i].is_initialized(), input[i].end());
        size_t next_i = i != cpus.size() - 1 ? i + 1 : 0;
        input[next_i][1] = output;
        if (i == cpus.size() - 1 && !cpus.back().is_halt()) {
          res = max(res, output);
        }
      }
    }
